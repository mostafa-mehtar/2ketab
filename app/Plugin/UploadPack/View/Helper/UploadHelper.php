...
class UploadHelper extends AppHelper {
    ...
    public function url($data, $field, $options = array())
    {
        ...
        {
            $settings = UploadBehavior::interpolate($model, $data, $id, $field, $filename, $options['style'], array('webroot' => ''));
            $url = isset($settings['url']) ? $settings['url'] : $settings['path'];
        }
        else
        {
            $settings = UploadBehavior::interpolate($model, $data, null, $field, null, $options['style'], array('webroot' => ''));
