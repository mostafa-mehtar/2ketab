...
class UploadBehavior extends ModelBehavior {
    ...
    private function _prepareToDeleteFiles(&$model, $field = null, $forceRead = false) {
        ...
        if ($needToRead) {
            $data = $model->find('first', array('conditions' => array($model->alias . '.' . $model->primaryKey => $model->id), 'callbacks' => false));
        } else {
            $data = $model->data;
        }
        if (is_array($this->toDelete)) {
            $this->toDelete = array_merge($this->toDelete, $data);
        } else {
            $this->toDelete = $data;
        }
    ...
    private function _deleteFiles(&$model) {
        foreach (self::$__settings[$model->name] as $field => $settings) {
            if (!empty($this->toDelete[$model->name][$field . '_file_name'])) {
                $styles = array_keys($settings['styles']);
                $styles[] = 'original';
                foreach ($styles as $style) {
                    $settings = $this->_interpolate($model, $field, $this->toDelete[$model->name][$field . '_file_name'], $style);
    ...
    private function _interpolate(&$model, $field, $filename, $style) {
        // must read this for fetch all data
        if(!empty($this->toDelete)){
            $model->data = $this->toDelete;
        }else{
            $model->data = $model->read();
        }
        
        return self::interpolate($model->name, $model->data, $model->id, $field, $filename, $style);
    }

    static public function interpolate($modelName, $data, $modelId, $field, $filename, $style = 'original', $defaults = array()) {
        if(!empty(self::$__settings[$modelName][$field]['folder'])){
            list($modelFolder, $folder_name) = explode('.', self::$__settings[$modelName][$field]['folder']);
        }
        ...
        $interpolations = array_merge(array(
            ...
            'id' => $modelId,
            'folder'=> (!empty(self::$__settings[$modelName][$field]['folder']))?$data[$modelFolder][$folder_name]:'',
            'style' => $style,
